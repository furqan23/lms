...
import 'package:splashapp/view_model/Controller/coursedetail_controller/homedetail_controller.dart';
...
class _VideoViewState extends State<HomeDetail> {
          ...
          actions: [
            Padding(
              ...
              child: Badge(
                ...
                child: IconButton(
                  onPressed: () async {
                    var updatedCartList = await Get.to(() => CartScreen(cartList: cartList));
  ...
  void onAddToDoubleCart(int index) {
    // Retrieve the course model using the provided index
    Data courseToAdds = homeDetailsController.coursemodel.value![index];

    // Check if the registration method is "whole"
    if (courseToAdds.registrationMethod == "whole") {
      // Ensure courseToAdds is not null
      if (courseToAdds!= null) {
        // Iterate over all courses in the courseToAdds object
        for (var course in courseToAdds.courses!) {
          final cartItem = CartModel(
            registrationMethod: courseToAdds.registrationMethod.toString(),
            categoryname: courseToAdds.catName.toString(),
            groupname: courseToAdds.name.toString(),
